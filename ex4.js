...
selectAllBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        // Seleccionamos todos los botones de intervalo y les añadimos la clase
...
deselectAllBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        // Seleccionamos todos los botones de intervalo y les quitamos la clase
        document.querySelectorAll('.acc-btn').forEach(b => {
...
function generateChallenge() {
    ...
    addNaturalsAfterAccidentals(targetSequenceNotes);
    drawTargetStaff(targetSequenceNotes);
    drawSungStaff(sungSequenceNotes); 
}

//Detectar alteracions per pintar becaires
function addNaturalsAfterAccidentals(sequenceNotes) {
    for (let i = 0; i < sequenceNotes.length; i++) {
        const current = sequenceNotes[i];
        
        // Si la nota actual es natural
        if (current.acc === 'n') {
            const currentNoteName = current.key.split('/')[0];
            
            // Buscar si en CUALQUIER punto anterior aparece la misma nota alterada
            let hasAlteredBefore = false;
            for (let j = 0; j < i; j++) {
                const previous = sequenceNotes[j];
                const previousNoteName = previous.key.split('/')[0];
                
                // Si encontramos la misma nota con alteración antes
                if (currentNoteName === previousNoteName && previous.acc !== 'n') {
                    hasAlteredBefore = true;
                    break;
                }
            }
            
            if (hasAlteredBefore) {
                current.showNatural = true;
            }
        }
    }
}
...
function yinDetector(buffer, sampleRate) {
    ...
    for (let tau = 0; tau < size; tau++) {
        for (let i = 0; i < size; i++) {
            //funció de diferència quadrada mitjana
    ...
    for (let tau = 1; tau < size; tau++) {
        ...
        //normalització de la mitjana acumulativa
...
function drawTargetStaff(sequenceNotes) {
    ...
    const notes = sequenceNotes.map(noteObj => {
        ...
        if (noteObj.acc !== "n") {
            staveNote.addModifier(new Accidental(noteObj.acc));
        } else if (noteObj.showNatural) {
