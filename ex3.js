...
let isFirstChallengeStarted = false;
...
// Close when clicking on the overlay background (not the inner div)
tipsDropdown.addEventListener('click', (event) => {
    if (event.target === tipsDropdown) {
        tipsDropdown.classList.add('hidden');
        if (!isFirstChallengeStarted) {
            generateChallenge();
            isFirstChallengeStarted = true;
            document.getElementById('startTipText').style.display = 'none';
        }
    }
});
...
noteBtn2.addEventListener('click', () =>{
    stopMic();
    drawComparison(sungChordNotes, currentChordMidi);
})
...
function drawComparison(userNotesArray, solutionNotesArray) {
    const container = document.getElementById('sheetMusic');
    container.innerHTML = "";
    ...
    // ACORDE DEL USUARIO
    let userNotes = userNotesArray ? [...userNotesArray] : [];
    if (currentWrongMidi && currentWrongMidi.length > 0) {
        userNotes = userNotes.concat(currentWrongMidi);
    }
    userNotes.sort((a, b) => a - b);

    const userDisplayNotes = userNotes.map(midi => isTenor ? midi + 12 : midi);
    const userKeysArray = userDisplayNotes.map(midi => midiToVexFlowString(midi));


    const userChordNote = new StaveNote({ 
        keys: userKeysArray, 
        duration: "w", 
        clef: clef 
    });
    
    userKeysArray.forEach((key, index) => {
        if (key.includes('#')) {
            userChordNote.addModifier(new Accidental("#"), index);
        }
        
        const originalMidi = userNotes[index]; 
        
        if (currentWrongMidi.includes(originalMidi)) {
            userChordNote.setKeyStyle(index, { fillStyle: "#ca483a", strokeStyle: "#ca483a" })
                        .setLedgerLineStyle({strokeStyle:"#ca483a"}); 
        } else {
            userChordNote.setKeyStyle(index, { fillStyle: "#629056", strokeStyle: "#629056" })
                        .setLedgerLineStyle({strokeStyle:"#629056"}); 
        }
    });

    // ACORDE SOLUCIÓN (EN NEGRO)
    let solutionNotes = solutionNotesArray ? [...solutionNotesArray] : [];
    solutionNotes.sort((a, b) => a - b);

    const solutionDisplayNotes = solutionNotes.map(midi => isTenor ? midi + 12 : midi);
    const solutionKeysArray = solutionDisplayNotes.map(midi => midiToVexFlowString(midi));

    const solutionChordNote = new StaveNote({ 
        keys: solutionKeysArray, 
        duration: "w", 
        clef: clef 
    });
    
    solutionKeysArray.forEach((key, index) => {
        if (key.includes('#')) {
            solutionChordNote.addModifier(new Accidental("#"), index);
        }
        solutionChordNote.setKeyStyle(index, { fillStyle: "#000000", strokeStyle: "#000000" })
                        .setLedgerLineStyle({strokeStyle:"#000000"}); 
    });

    // DIBUJAR AMBOS ACORDES EN EL MISMO PENTAGRAMA
    const voice = new Voice({ num_beats: 8, beat_value: 4 });
    voice.addTickables([userChordNote, solutionChordNote]);
    
    const availableSpace = stave.getNoteEndX() - stave.getNoteStartX();
    const startX = stave.getNoteStartX() + (availableSpace - 100) / 2;
    stave.setNoteStartX(startX);

    new Formatter().format([voice], 100);
    voice.draw(context, stave);
